"""Tool handler for explaining code using Ollama."""
...
def explain_code(filename: Optional[str] = None) -> dict[str, str | bool]:
	"""Explain a Python file using Ollama.

	If filename is not provided, uses the most recently modified file.
	Returns {success, explanation, filename}
	"""
	OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

	# Resolve which file to explain
	if filename:
	...
	if not _is_within_output(target_path):
		return {
			...
			"message": f"Refused to read file outside output directory: {filename}",
		}
	...
	try:
		...
		explanation = str(response_data.get("response", "")).strip()
		...
		if not explanation:
			return {
				...
				"explanation": "",
				"filename": selected_filename,
				"message": "Model returned an empty explanation.",
			}
		...
		return {
			...
			"explanation": "",
			"filename": selected_filename,
			"message": f"Failed to explain code in {selected_filename}: {exc}",
		}
